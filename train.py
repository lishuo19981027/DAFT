...
if __name__ == '__main__':
    ...
    parser.add_argument('--features_path', type=str, default='/media/a1002/two/1/feature/coco_X101_grid.hdf5')
    parser.add_argument('--pixel_path', type=str, default='/home/a1002/wyh/segmentations')
    #parser.add_argument('--pixel_path', type=str, default='/media/yuhang/3E66E7FF66E7B5B5/dataset/coco_detections/coco_detections.hdf5')
    parser.add_argument('--annotation_folder', type=str, default='/media/a1002/two/1/annotation')
    # parser.add_argument('--annotation_folder', type=str, default='/media/a1002/455a192b-90a4-471d-9e73-6722900a0aad/dataset/wyh/annotation')
    ...
    # Pipeline for pixel
    # pixel_field = ImageDetectionsField(detections_path=args.features_path, max_detections=49, load_in_tmp=False)
    ...
    if args.resume_last or args.resume_best:
        if args.resume_last:
            fname = os.path.join(args.model_path, '%s_last9.pth' % args.exp_name)
        else:
            fname = os.path.join(args.model_path, '%s_best9.pth' % args.exp_name)
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if switch_to_rl and not best:
            data = torch.load(os.path.join(args.model_path, '%s_best9.pth' % args.exp_name))
            torch.set_rng_state(data['torch_rng_state'])
            torch.cuda.set_rng_state(data['cuda_rng_state'])
            np.random.set_state(data['numpy_rng_state'])
            random.setstate(data['random_rng_state'])
        ...
        torch.save({
            ...
            'use_rl': use_rl,
        }, os.path.join(args.model_path, '%s_last9.pth' % args.exp_name))

        if best:
            copyfile(os.path.join(args.model_path, '%s_last9.pth' % args.exp_name), os.path.join(args.model_path, '%s_best9.pth' % args.exp_name))
        if exit_train:
            writer.close()
            break
